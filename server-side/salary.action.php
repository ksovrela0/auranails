...
switch ($act){
    ...
    case 'get_list_salary':
        ...
        $db->setQuery(" SELECT CONCAT(personal.`name`, ' ', personal.lastname) AS fullname,
                                COUNT(procedures.id) AS proc_count,
                                ROUND(SUM(procedures.price)*(procedures.salary_percent/100),2) AS salary,
                                ROUND(SUM(procedures.price)) AS total
                                
                        FROM personal
                        JOIN procedures ON procedures.actived = 1 AND procedures.user_id = personal.id AND procedures.status_id = 2
                        JOIN orders ON orders.id = procedures.order_id AND orders.actived = 1
                        WHERE orders.write_date BETWEEN '$start_Date' AND '$end_Date'
